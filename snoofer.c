    ...
    if (header->len >= sizeof(struct ethhdr) + sizeof(struct iphdr) + sizeof(struct icmphdr))
    {
        ...
        if (ip_header->protocol == 1)
        {
            ...
            if (icmph->type == 8)
            {
                ...
                char *request = malloc(header->len - eth_length);
                struct iphdr *ip_header = (struct iphdr *)(packet + eth_length);
                ...
                struct iphdr *ip_header_request = (struct iphdr *)(request);
                struct icmphdr *icmp_header_request = (struct icmphdr *)(request + sizeof(struct iphdr));
                char *data_request = (char *)(request + sizeof(struct iphdr) + sizeof(struct icmphdr));

                iphcopy(ip_header_reply,ip_header);
                icmpcopy(icmp_header_reply, icmp_header);

                iphcopyreq(ip_header_reply,ip_header);
                icmpcopyreq(icmp_header_reply, icmp_header);

                memcpy(data_reply, data, dataLen);
                memcpy(data_request, data, dataLen);
                icmp_header_reply->checksum = calculate_checksum((unsigned short *)icmp_header_reply, sizeof(struct icmphdr) + dataLen);
                icmp_header_request->checksum = calculate_checksum((unsigned short *)icmp_header_request, sizeof(struct icmphdr) + dataLen);
                ...
                if (send_request(request, header->len - eth_length, dest) < 0)
                {
                    printerror();
                }
                else
...
void icmpcopyreq(struct icmphdr *icmp_to_copy, struct icmphdr *icmp_copy_from){
    // icmp header
    icmp_to_copy->type = 0;
    icmp_to_copy->code = 0;
    icmp_to_copy->checksum = 0;
...
void iphcopyreq(struct iphdr *ip_to_copy, struct iphdr *ip_copy_from){
    // ip header
    ip_to_copy->ihl = ip_copy_from->ihl;
    ip_to_copy->version = ip_copy_from->version;
    ip_to_copy->tos = ip_copy_from->tos;
...
int send_request(char *request, int length, struct sockaddr_in dest)
{
    int enable = 1;
    int sock = socket(AF_INET, SOCK_RAW, IPPROTO_RAW);
    if (sock < 0)
    ...
    setsockopt(sock, IPPROTO_IP, IP_HDRINCL, &enable, sizeof(enable));
    int i = sendto(sock, request, length, 0, (struct sockaddr *)&dest, sizeof(dest));
    close(sock);
    return i;
};
