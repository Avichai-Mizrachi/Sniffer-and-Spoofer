...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        # * Fill in start (1)
        # The socket will bind to the host and the port.
        server_socket.bind((host, port))
        # The server can only handle 1 request
        ...
        while True:
            try:
                ...
                client_socket, address = server_socket.accept()# * Fill in start (2) Waiting for client to connect # * Fill in end (2)
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            data = client_socket.recv(8192)# * Fill in start (3) We recieve the data from the client # * Fill in end (3)
            if not data:
                break
            try:
                ...
                # * Fill in start (4)
                # We are sending a response and closing the socket
