...
char *my_device(char *ebuffer, pcap_t *handle);
...
int main()
{
    ...
    char *devName = my_device(ebuffer, handle); // Select the network interface to sniff on
    ...
    // catch errors
    int result2 = pcap_setfilter(handle, filter_exp);
    if (result2 == 0) {
